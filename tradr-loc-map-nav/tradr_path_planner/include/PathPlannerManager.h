...
//#include <pcl_ros_c11/filters/crop_box.h>
//#include <pcl_ros_c11/CropBoxConfig.h>
...
public:
    ...
    // FIXME: this must be kept in synch with tradr_trajectory_control_msgs::PlanningStatus
    enum PlannerStatus
    {
        kNone             = 0,
        kNotReady         = 1,
...
public: /// < setters 
   ...
   // set the abort flag for possibly aborting current planning 
    ...
    { 
        b_abort_ = val; 
        if(p_tradr_path_planner_) 
        {
            p_tradr_path_planner_->setAbort(val);
...
protected: 
    ...
    boost::shared_ptr<PathPlanner> p_tradr_path_planner_; // the used path planner instance
    boost::recursive_mutex tradr_path_planner_mutex_;
