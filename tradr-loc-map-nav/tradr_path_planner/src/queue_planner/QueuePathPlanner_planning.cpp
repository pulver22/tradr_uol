...
void QueuePathPlanner::pathPlanningCallback(TaskSegmentPtr segment, TaskPtr task)
{
    ...
    boost::shared_ptr<PathPlanner> p_tradr_path_planner(new PathPlanner);
    segment->p_tradr_path_planner = p_tradr_path_planner;
    segment->p_tradr_path_planner->setCostFunctionType(cost_function_type_,lambda_trav_,lambda_utility_2d_,tau_exp_decay_);
    ...
    while ((segment->crop_step < PathPlannerManager::kNumCropBoxMethod) && (!b_successful_planning) && (!segment->b_aborted))
    {
        ...
        p_tradr_path_planner->setInput(*segment->p_traversability_pcl, *segment->p_wall_pcl, segment->wall_kdtree, segment->traversability_kdtree, pointIdxNKNSearch[0]);
        if(utility_2d_flag_) p_tradr_path_planner->set2DUtility(*segment->p_utility_2d_pcl);
        p_tradr_path_planner->setGoal(goal);
        ...
        /// < compute the path
        b_successful_planning = p_tradr_path_planner->planning(segment->path);
