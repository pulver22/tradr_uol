...
{
    p_tradr_path_planner_.reset(new PathPlanner);
...
{        
    boost::recursive_mutex::scoped_lock path_planning_locker(tradr_path_planner_mutex_);
    ...
    {
        pcl::PointXYZI goal_position = goal_position_; //p_tradr_path_planner_->getGoal(); // do not get the goal from the path planner since it is still not set 
    ...
    while ((num_failures < kPathPlannerMaxNumAttempts) && (!b_successful_planning) && (!b_abort_))
    {
        /// < first set input then the goal!
        {
            ...
            p_tradr_path_planner_->setInput(*p_traversability_pcl, wall_pcl_, wall_kdtree_, traversability_kdtree, pointIdxNKNSearch[0]);
            if(b_utility_2d_info_available_) p_tradr_path_planner_->set2DUtility(*p_utility_2d_pcl);
        }
        
        /// < set goal and check if close enough to the traversability map
        bool b_goal_accepted = p_tradr_path_planner_->setGoal(goal_position_);
        if(!b_goal_accepted)
        {
        ...
        /// < compute the path
        b_successful_planning = p_tradr_path_planner_->planning(path_);
        if (!b_successful_planning)
        {
            ...
            if (crop_step < kNumCropBoxMethod)
            {
                ...
                {
                    boost::recursive_mutex::scoped_lock goal_locker(goal_mutex_);
                    pcl::PointXYZI goal_position = p_tradr_path_planner_->getGoal(); // get the goal from the path planner since it is set 
                    distance2D_to_goal = PathPlanner::dist2D(goal_position, robot_position);
                    distance3D_to_goal = PathPlanner::dist(goal_position, robot_position);
                    std::cout << "PathPlannerManager::doPathPlanning() - distance to goal - 3D: " << distance3D_to_goal <<", 2D: " << distance2D_to_goal << std::endl;
                }
...
PathPlannerManager::PlannerStatus PathPlannerManager::pathPlanningServiceCallback(const geometry_msgs::PoseStamped start, const geometry_msgs::PoseStamped end, nav_msgs::Path& path_out, double& path_cost)
{
    ...
    while ((num_failures < kPathPlannerMaxNumAttempts) && (!b_successful_planning) && (!b_abort_))
    {
        ...
        /// < set goal and check if close enough to the traversability map
        bool b_gool_accepted = p_tradr_path_planner_->setGoal(end_position);
        if(!b_gool_accepted)
        {
        ...
        /// < compute the path
        b_successful_planning = p_tradr_path_planner_->planning(path_out);
        if (!b_successful_planning)
        {
...
void PathPlannerManager::setCostFunctionType(int type, double lamda_trav, double lambda_aux_utility)
{
    ...
    p_tradr_path_planner_->setCostFunctionType(type, lamda_trav, lambda_aux_utility);
}
