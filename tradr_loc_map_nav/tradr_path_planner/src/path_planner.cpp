...
boost::shared_ptr< actionlib::SimpleActionClient<tradr_trajectory_control_msgs::TrajectoryControlAction> > p_act_client;

// current path planner failures in computing a path towards the selected goal 
int tradr_path_planner_num_failures = 0;
    ...
    if (is_traversability_info_available && is_wall_info_available)
    {
        ...
        if (is_goal_selected)
        {
            ...
            if( (distance3D_to_goal < kAcceptable3DDistanceToGoal) && (distance2D_to_goal < kAcceptable2DDistanceToGoal) )
            {
                ...
                tradr_path_planner_num_failures = 0;
                return; /// < EXIT POINT
            }
            ...
            if (is_goal_selected)
            {
                if (p_planner->planning(path))
                {
                    ...
                    tradr_path_planner_num_failures = 0; // reset num failures  
                ...
                else
                {
                    ...
                    tradr_path_planner_num_failures++; // increase num failures 
                    ROS_INFO("path planner failure num %d", tradr_path_planner_num_failures);
                    if (tradr_path_planner_num_failures > kPathPlannerMaxNumAttempts)
                    {
                        is_goal_selected = false;
                        tradr_path_planner_num_failures = 0;
...
{
    ros::init(argc, argv, "tradr_path_planner_node");
    ...
    std::string tradr_trajectory_control_node_name = getParam<std::string>(n, "tradr_trajectory_control_action_server_name", "tradr_trajectory_control_action_server_ugv1");
    ...
    p_act_client.reset(new actionlib::SimpleActionClient<tradr_trajectory_control_msgs::TrajectoryControlAction>(tradr_trajectory_control_node_name,true)); 
