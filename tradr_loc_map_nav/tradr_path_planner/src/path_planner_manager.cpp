...
#include <tradr_trajectory_control_msgs/PlanningFeedback.h>
#include <tradr_trajectory_control_msgs/PlanningTask.h>
#include <tradr_trajectory_control_msgs/PlanningGlobalPath.h>
#include <tradr_trajectory_control_msgs/PlanningStatus.h>
#include <tradr_trajectory_control_msgs/PathPlanning.h>
...
ros::Publisher tradr_trajectory_control_abort_pub;
...
public:
    ...
    tradr_trajectory_control_msgs::PlanningGlobalPath path_;
...
void globalPathCallback(const tradr_trajectory_control_msgs::PlanningGlobalPath& global_path_msg);
...
void setDefinitiveFailure()
{
    ...
    tradr_trajectory_control_msgs::PlanningStatus msg_plan_status;
    msg_plan_status.success = false;
    msg_plan_status.status  = tradr_trajectory_control_msgs::PlanningStatus::kFailure;
...
void doPathPlanning()
{
    ...
    case PathPlannerManager::kTransformFailure:
        ...
        msg_abort.data = true;
        tradr_trajectory_control_abort_pub.publish(msg_abort);
        }
    ...
    case PathPlannerManager::kSuccess:
        ...
        msg_plan_status.status    = tradr_trajectory_control_msgs::PlanningStatus::kSuccess;
...
//            }
        ...
        {
        msg_plan_status.success = true;
        msg_plan_status.status  = tradr_trajectory_control_msgs::PlanningStatus::kArrived;
        path_plan_stat_pub.publish(msg_plan_status);
        boost::recursive_mutex::scoped_lock time_locker(last_pp_success_timemutex_);
        time_last_path_planning_success = ros::Time::now();
        }
    ...
    switch(planning_status)
    {
        case PathPlannerManager::kSuccess:
        {
...

            {
                ...
                tradr_trajectory_control_msgs::PlanningStatus msg_plan_status;
                msg_plan_status.success = true;
                msg_plan_status.status  = tradr_trajectory_control_msgs::PlanningStatus::kFirstSuccess;
            ...
            if(path_size > 0)
            {
                tradr_trajectory_control_msgs::PlanningGlobalPath global_path_msg;
...
void feedbackCallback(const tradr_trajectory_control_msgs::PlanningFeedback& feedback_msg)
{
    // if the message has been published by the tool node, then
    if (!feedback_msg.node.compare("tool"))
    {
        tradr_trajectory_control_msgs::PlanningFeedback feedback_msg;
...
bool pathPlanningServiceCallback(tradr_trajectory_control_msgs::PathPlanning::Request  &req, tradr_trajectory_control_msgs::PathPlanning::Response &res)
{
...
{
    ros::init(argc, argv, "tradr_path_planner_manager_node");
    ...
    path_plan_stat_pub = n.advertise<tradr_trajectory_control_msgs::PlanningStatus>("/path_planning_status", 1);

    goal_abort_pub = n.advertise<std_msgs::Bool>("/goal_abort_topic", 1);
    tradr_trajectory_control_abort_pub = n.advertise<std_msgs::Bool>("/tradr_trajectory_control_abort_topic", 1);

    // feedback between nodes: "tool", "planner" and "control"
    queue_task_feedback_pub = n.advertise<tradr_trajectory_control_msgs::PlanningFeedback>(queue_task_feedback_topic, 1);
