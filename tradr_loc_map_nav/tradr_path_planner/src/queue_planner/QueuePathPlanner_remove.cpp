...
void QueuePathPlanner::removeCallback(const tradr_trajectory_control_msgs::PlanningTask& task_msg)
{ 
    ...
    for( std::list<TaskPtr>::iterator it=task_queue_.begin(); task_queue_.end()!=it; )
    {
        ...
        if( !name.compare(task_msg.name) || !task_msg.name.compare("ALL") )
        {
            ...
            for( size_t i=0; task->size()>i; i++ )
            {
                ...
                if(segment->p_tradr_path_planner) segment->p_tradr_path_planner->setAbort(true); 
